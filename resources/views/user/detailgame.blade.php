            ...
            <div class="pt-6">
                <div class="mx-auto mt-6 max-w-2xl sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:gap-x-8 lg:px-8">
                    <div class="aspect-h-5 aspect-w-4 lg:aspect-h-4 lg:aspect-w-3 sm:overflow-hidden sm:rounded-lg">
                        <img src="{{ $game->gambar }}" alt="Model wearing plain white basic tee."
                ...
                <div
                    ...
                    <div class="mt-4 lg:row-span-3 lg:mt-0">
                        ...
                        <form class="mt-10" action="/user/checkout" method="POST">
                            @csrf
                            <!-- Sizes -->
                            <div class="mt-10">
                                    ...
                                    <fieldset class="w-full space-y-1 text-gray-800">
                                        ...
                                        <div class="flex px-2">
                                            <input type="text" name="nickname" id="nickname"
                                                ...
                                                class="flex py-1 flex-1 border sm:text-sm rounded-md focus:ri border-gray-300 text-gray-800 px-4 bg-gray-100 focus:ri">
                                        </div>
                                    </fieldset>
                                </div>
                            </div>
                                                ...
                                                @if ($diamonds->count() > 0)
                                                    <select name="qty" id="qty">
                                                        @foreach ($diamonds as $d)
                                                            <option value="{{ $d->harga }}">{{ $d->nominal }} Diamond -
                                                                Harga Rp {{ $d->harga }}</option>
                                                        @endforeach
                                                    </select>
                                                @endif
                                            </div>
                            ...
                            <input type="hidden" name="game" id="game" value="{{ $game->name_game }}">

                            <button type="submit"
                                class="mt-10 flex w-full items-center justify-center rounded-md border border-transparent bg-indigo-600 px-8 py-3 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Checkout</button>
                        </form>
                    </div>
                        ...
                        <div class="mt-10">
                            ...
                            <div class="mt-4">
                                <ul role="list" class="list-disc space-y-2 pl-4 text-sm">
                                    @if ($diamonds->count() > 0)
                                        @foreach ($diamonds as $d)
                                            <li class="text-gray-400"><span class="text-gray-600">Jumlah
                                                    {{ $d->material }} = {{ $d->nominal }} - Harga
                                                    {{ $d->material }} = Rp{{ $d->harga }}</span></li>
                                        @endforeach
                                    @endif
                                </ul>
                            </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            qtySelect.addEventListener("change", function() {
                ...
                const selectedHarga = selectedOption.value;
